...
/// Identifier of the mongodb-driver
pub(in crate::storage::mongodb) const DRIVER_ID: &str = "mongodb";
...
impl MongoDBPool {
    ...
    ///
    /// Returns [`ServerConnectionError`](DBError::ServerConnectionError)
    /// if it's not able to reach the database-server.
    ///
    /// Returns [`MigrationRequiredError`](DBError::MigrationRequiredError)
    /// if the database uses an older version of the schema.
    ///
    /// Returns [`QueryError`](DBError::QueryError)
    ...
    pub async fn new(uri: (String, String), cred: (String, String))
        ...
        let client = get_client(uri, cred).await?;
...
/// Attempts to connect to a MongoDB-server and returns a [`Client`]-object if successful.
///
/// On failure a [`ServerConnectionError`](DBError::ServerConnectionError) will be returned.
///
/// # Arguments
///
/// * `uri` - Tuple of Strings containing the url and port of the database-server
/// * `cred` - Tuple of Strings containing the username and password of a database-user
async fn get_client(uri: (String, String), cred: (String, String)) -> Result<Client, DBError> {
    ...
    if db.run_command(doc! {"ping": 1}, None).await.is_ok() {
        Ok(client)
    } else { Err(DBError::ServerConnectionError)  }
}
